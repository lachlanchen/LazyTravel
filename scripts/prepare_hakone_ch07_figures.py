#!/usr/bin/env python3
"""Prepare reproducible print/web variants and provenance for Hakone Chapter 7."""
...
CONFIG_PATH = ROOT / "data/images/hakone/ch07-figures.config.json"
...
def path_record(path: Path, *, include_dimensions: bool = False) -> dict[str, Any]:
    record: dict[str, Any] = {
        "path": str(path) if path.is_absolute() and not path.is_relative_to(ROOT) else str(path.relative_to(ROOT)),
        "sha256": sha256(path),
...
def bottom_crop(image: Image.Image, size: tuple[int, int]) -> Image.Image:
    ...
    if source_ratio > target_ratio:
        ...
        box = (0, image.height - crop_height, image.width, image.height)
...
def main() -> int:
    ...
    for figure in config["figures"]:
        ...
        with Image.open(raw_path) as opened:
            ...
            base = resize_width(source, BASE_WIDTH)
            wide = bottom_crop(source, WIDE_SIZE)
            web = bottom_crop(source, WEB_SIZE)

        stem = OUTPUT_DIR / figure["stem"]
        base_path = stem.with_suffix(".jpg")
        wide_path = stem.with_name(f"{stem.name}-wide.jpg")
        web_path = stem.with_name(f"{stem.name}-web.jpg")
        save_jpeg(base, base_path, 92)
        save_jpeg(wide, wide_path, 92)
        save_jpeg(web, web_path, 88)
        ...
        provenance = {
            ...
            "postprocess": "Pillow sRGB conversion, metadata-free width resize, bottom-anchored 16:9 print/web crops, Lanczos resampling, JPEG quality 92; web quality 88",
            "output": {**path_record(base_path, include_dimensions=True), "color_space": "sRGB"},
            "variants": {
                "print_wide": path_record(wide_path, include_dimensions=True),
                "web": path_record(web_path, include_dimensions=True),
            },
            "factual_limits": figure["factual_limits"],
            "reader_caption_policy": figure["reader_caption_policy"],
            "rights": "Original LazyTravel composition; external character references are hash-pinned and not redistributed",
            "visual_qa": qa,
        }
        ...
        print(f"figure: {base_path.relative_to(ROOT)}")
