#!/usr/bin/env python3
"""Build, validate, and optionally sync the current Lanzhou B6 pocket review."""
...
COVER_ASSET = ROOT / "assets/images/lanzhou/lanzhou-cover-underlay.png"
COVER_PROVENANCE = COVER_ASSET.with_suffix(".provenance.json")
BUILD_DIR = ROOT / "build/books/lanzhou/pocket-review"
DIST_DIR = ROOT / "dist/books/lanzhou"
DIST_PDF = DIST_DIR / "lanzhou-pocket-review.pdf"
DIST_MANIFEST = DIST_DIR / "lanzhou-pocket-review.manifest.json"
NUTSTORE_PDF = (
...
def active_chapter_ids(document: dict[str, Any]) -> list[str]:
    ...
    if len(chapters) != EXPECTED_CHAPTERS:
        raise RuntimeError(
            f"Lanzhou structure must remain exactly {EXPECTED_CHAPTERS} chapters"
        )
    active = [chapter for chapter in chapters if chapter["blocks"]]
    if not active:
        raise RuntimeError("Lanzhou has no compiled chapter content")
    expected_orders = list(range(1, len(active) + 1))
    if [chapter["order"] for chapter in active] != expected_orders:
        raise RuntimeError("compiled Lanzhou chapters must form a consecutive prefix")
...
def validate_variant_hash(path: Path, provenance: dict[str, Any]) -> None:
    ...
    candidates.extend(provenance.get("outputs", {}).values())
    record = next(
...
def validate_asset_qa(document: dict[str, Any], chapter_ids: list[str]) -> None:
    ...
    for asset_id in sorted(used_asset_ids):
        ...
        if missing_guides:
            raise RuntimeError(
                f"non-map asset lacks one or more four-guide references: {asset_id}: "
...
def validate_cover_qa() -> None:
    if not COVER_ASSET.is_file() or not COVER_PROVENANCE.is_file():
        raise RuntimeError("Lanzhou cover underlay or provenance is missing")
    ...
    if output.get("sha256") != sha256(COVER_ASSET):
        raise RuntimeError("Lanzhou cover provenance hash mismatch")
    if (output.get("width"), output.get("height")) != (1476, 2079):
        raise RuntimeError("Lanzhou cover is not the required 300 ppi B6 geometry")
    visual = provenance.get("visual_qa", {})
    required = (
    ...
    if not visual.get("approved") or any(visual.get(field) != "pass" for field in required):
        raise RuntimeError("Lanzhou cover has not passed compiled-page visual QA")
...
def build_inputs(document: dict[str, Any], chapter_ids: list[str]) -> list[Path]:
    inputs = {
        ...
        ROOT / "scripts/build_lanzhou_nearby_day_map.py",
        ROOT / "scripts/build_lanzhou_onward_gates_map.py",
        ...
        ROOT / "scripts/prepare_lanzhou_ch10_figures.py",
        ROOT / "scripts/prepare_lanzhou_ch11_figures.py",
    ...
    for folder in (ROOT / "data/maps/lanzhou", ROOT / "data/images/lanzhou"):
...
def main() -> int:
    ...
    if not args.skip_map:
        ...
        run([sys.executable, "scripts/build_lanzhou_museum_route_map.py"])
        ...
        run([sys.executable, "scripts/build_lanzhou_nearby_day_map.py"])
        run([sys.executable, "scripts/build_lanzhou_onward_gates_map.py"])
    ...
    if page_count < 14:
        raise RuntimeError(f"unexpectedly short Lanzhou review PDF: {page_count} pages")
    text = run(["pdftotext", str(pdf), "-"])
    text_characters = len("".join(text.split()))
    if text_characters < 12_000:
        raise RuntimeError(
            f"Lanzhou PDF text layer is unexpectedly short: {text_characters} characters"
        )
    for marker in ("兰州旅行手册", "蘭州旅の手引き", "Lanzhou Pocket Travel Guide"):
        if marker not in text:
            raise RuntimeError(f"Lanzhou PDF text layer is missing live cover text: {marker}")
